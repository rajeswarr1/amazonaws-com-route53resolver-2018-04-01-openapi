...
func TagresourceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=Route53Resolver.TagResource", cfg.BaseURL)
...
func CreateTagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=Route53Resolver_TagResource",
		mcp.WithDescription("Adds one or more tags to a specified resource."),
